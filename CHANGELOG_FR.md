...
1. Ajout de l’option facultative « Créer l’audiodescription dans le fichier vidéo d’origine ». Lorsqu’elle est activée, Sonarpad privilégie désormais le MP4 : le flux vidéo d’origine est copié sans réencodage et la piste d’audiodescription mixée est ajoutée. Si FFmpeg signale une incompatibilité de conteneur ou d’écriture des paquets lors de la création du MP4, Sonarpad réessaie automatiquement la même exportation en MKV, toujours sans réencoder la vidéo. Il reste aussi possible de choisir explicitement MKV. Les pauses étendues sont ignorées dans ce mode afin de conserver la synchronisation audio-vidéo.

2. Amélioration de la réanalyse des segments dans les projets d’audiodescription enregistrés. La fenêtre utilise désormais l’accès IA déjà configuré dans la fenêtre principale Créer une audiodescription ; la clé API, le crédit Sonarpad AI et le choix du modèle ne sont donc plus dupliqués. Un segment réanalysé conserve maintenant tout le groupe de descriptions du projet appartenant à ce segment d’analyse au lieu de seulement la première ou la plus proche ; toutes les descriptions du groupe sont marquées comme réanalysées et « Appliquer le segment et réexporter le MP3 » applique tout le groupe en une seule opération. Les aperçus des pauses étendues sont synthétisés directement au lieu de rechercher leur position dans le MP3 exporté, ce qui évite les aperçus qui commencent par l’audio du film ou coupent la description.

3. Le fallback média conservateur de Gemini a été amélioré sans modifier le fonctionnement déjà opérationnel. L’erreur HTTP 400 `INVALID_ARGUMENT` continue d’activer des segments MKV plus petits (environ 15 Mo), puis des segments MP4 compatibles. En plus, si Gemini accepte un segment téléversé mais que son traitement se termine par `FAILED`, Sonarpad retéléverse exactement ce segment une fois avant de passer au fallback MP4 ; les tentatives pour le Code 13 du serveur sont limitées à trois avant ce même fallback, afin d’éviter les attentes infinies. Les erreurs réseau, quota, authentification ou synthèse n’activent pas ce parcours.

4. Correction de la conservation des identifiants IA lors du changement de mode d’accès. La clé API Gemini personnelle et le code d’accès Sonarpad AI sont désormais enregistrés indépendamment : passer de la clé API personnelle à Sonarpad AI, ou inversement, n’efface plus l’identifiant inactif. La clé Gemini est également enregistrée lorsque son champ perd le focus.

5. Ajout d’un véritable bouton Annuler pour « Réanalyser le segment » et « Appliquer le segment et réexporter le MP3 ». La barre de progression reste active, tandis qu’Annuler interrompt désormais la préparation FFmpeg du segment, le worker IA, la validation TTS et l’export final dès que l’étape en cours peut s’arrêter en toute sécurité. L’application d’un segment réanalysé est désormais transactionnelle : le projet et le média existants ne sont remplacés qu’après une réexportation réussie ; en cas d’annulation, les fichiers précédents restent inchangés et la proposition réanalysée reste disponible pour une nouvelle tentative.

6. Correction de la réanalyse des segments pour les descriptions situées au-delà du premier chunk Gemini. Le segment isolé est désormais envoyé au worker existant avec une timeline locale commençant à 0, puis remappé sur sa position absolue dans le projet, ce qui évite `Invalid prepared chunk timeline at chunk 1` sans modifier le pipeline normal d’audiodescription.

7. La réanalyse des segments des projets enregistrés utilise désormais les mêmes contrôles de sécurité que la création complète d’une audiodescription. Le segment sélectionné passe par la même extraction audio mono 16 kHz et la même analyse dialogues/silences avec Pyannote, les mêmes règles temporelles et solutions de repli Gemini, la même synthèse TTS réelle avec la voix du projet et le même planificateur final pour le placement sûr et les pauses étendues. Lors de l’application, Sonarpad conserve les nouveaux temps absolus déjà vérifiés et actualise les intervalles de protection Pyannote du segment ; l’ancien contournement spécial de prévisualisation pour les textes réanalysés trop longs a été supprimé.

8. Correction du remplacement structurel du segment après la réanalyse complète. Sonarpad n’exige plus que le segment nouvellement vérifié contienne exactement le même nombre de descriptions que le projet enregistré : les anciennes descriptions du chunk sont remplacées par l’ensemble du résultat sûr de la pipeline complète, de sorte qu’un segment de 10 descriptions peut correctement passer à 9 (ou 11). L’éditeur affiche immédiatement le nouveau nombre et les nouveaux temps pour l’aperçu, tandis que le projet et le média enregistrés restent inchangés jusqu’à la réussite de « Appliquer le segment et réexporter le MP3 ».

9. La réanalyse d’un segment a été remaniée afin que le fragment physique sélectionné soit traité comme un véritable mini-film autonome. Sonarpad transmet désormais ce mini-film directement à exactement la même fonction `create_audio_description` que celle utilisée par la création complète normale : la vidéo et la piste audio partagent ainsi la même chronologie locale et les contrôles Pyannote, Gemini, TTS réel, planification et sécurité habituels s’exécutent sans implémentation de réanalyse distincte. Ce n’est qu’après cette pipeline normale que les temps locaux obtenus sont replacés à leur position d’origine dans le film.

Édition de texte
1. Amélioration de « Joindre les lignes coupées » dans Édition > Texte. Sans sélection, la commande traite tout le document ; avec une sélection, elle ne traite que les lignes sélectionnées. Elle reconstruit désormais des paragraphes complets en joignant les lignes consécutives même après une phrase terminée par une ponctuation ; les lignes vides restent des séparateurs de paragraphes et les listes numérotées ou à puces sont conservées.

10. Correction de la synchronisation temporelle lors du retour du mini-film réanalysé vers le film original. Sonarpad applique désormais la même petite réconciliation de durée des segments que l’analyse complète aux descriptions, aux horodatages Visual Evidence et aux intervalles de dialogue protégés, évitant un décalage progressif de quelques dixièmes de seconde vers les dialogues.
