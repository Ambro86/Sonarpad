...
Audiodescripción con IA
1. Se añadió una recuperación conservadora para la exportación multicanal sin cambiar la canalización de los archivos que ya funcionan. Sonarpad sigue usando primero la distribución de canales original y solo completa un último fotograma PCM incompleto cuando es necesario. Si aun así el WAV intermedio multicanal falla porque el número de muestras no es múltiplo del número de canales, Sonarpad vuelve a intentar automáticamente solo esa exportación en estéreo. Las exportaciones mono, estéreo y multicanal que ya funcionan no cambian.

Tema oscuro y accesibilidad
1. Corregido un problema por el que el tema oscuro interfería con los cuadros de diálogo nativos de Windows y los mensajes de confirmación. Sonarpad ya no aplica su subclass personalizado del tema oscuro a los árboles de diálogo del sistema `#32770`, por lo que Abrir/Guardar, la exportación de diagnósticos y los MessageBox siguen gestionados por Windows y reciben correctamente el foco de teclado/NVDA. También se corrigió la duración en memoria de la extensión ZIP predeterminada del cuadro Guardar de diagnósticos.

Versión 0.9.7 – 2026-09-11

Audiodescripción con IA
1. Se añadió la opción “Crear la audiodescripción en el archivo de vídeo original”. Cuando está activada, Sonarpad ahora prefiere MP4: copia el flujo de vídeo original sin recodificarlo e inserta la pista de audiodescripción mezclada. Si FFmpeg informa de una incompatibilidad del contenedor o de escritura de paquetes al crear el MP4, Sonarpad vuelve a intentar automáticamente la misma exportación como MKV, también sin recodificar el vídeo. También se puede elegir MKV de forma explícita. En este modo se ignoran las pausas extendidas para mantener sincronizados audio y vídeo.

2. Se mejoró el nuevo análisis de segmentos en los proyectos de audiodescripción guardados. La ventana usa ahora el acceso a IA ya configurado en la ventana principal Crear audiodescripción, por lo que ya no duplica la clave API, el crédito de Sonarpad AI ni la selección del modelo. Un segmento vuelto a analizar conserva ahora todo el grupo de descripciones del proyecto pertenecientes a ese fragmento de análisis, en lugar de solo la primera o la más cercana; todas las descripciones del grupo se marcan como vueltas a analizar y “Aplicar segmento y volver a exportar MP3” aplica todo el grupo de una vez. Las vistas previas de las pausas extendidas se sintetizan directamente en lugar de buscar su posición en el MP3 exportado, evitando que la vista previa empiece con audio de la película o corte la descripción.

3. Se mejoró el fallback conservador de medios de Gemini sin modificar el flujo que ya funciona. El HTTP 400 `INVALID_ARGUMENT` sigue activando fragmentos MKV más pequeños (unos 15 MB) y después fragmentos MP4 compatibles. Además, si Gemini acepta un fragmento cargado pero su procesamiento termina en `FAILED`, Sonarpad vuelve a cargar exactamente ese fragmento una vez y solo después usa el fallback MP4; los reintentos del Code 13 del servidor se limitan a tres antes del mismo fallback, evitando esperas infinitas. Los errores de red, cuota, autenticación o síntesis no activan este recorrido.

4. Corregida la persistencia de las credenciales de IA al cambiar el modo de acceso. La clave API personal de Gemini y el código de acceso de Sonarpad AI se guardan ahora de forma independiente: al pasar de Clave API personal a Sonarpad AI, o viceversa, ya no se borra la credencial inactiva. La clave de Gemini también se guarda cuando su campo pierde el foco.

5. Se añadió un verdadero botón Cancelar para “Volver a analizar segmento” y “Aplicar segmento y volver a exportar MP3”. La barra de progreso permanece activa y Cancelar ahora interrumpe la preparación FFmpeg del segmento, el proceso de IA, la validación TTS y la exportación final en cuanto la fase actual puede detenerse con seguridad. La aplicación de un segmento vuelto a analizar ahora es transaccional: el proyecto y el archivo multimedia existentes solo se sustituyen después de una reexportación correcta; si se cancela, los archivos anteriores quedan intactos y la propuesta vuelve a estar disponible para otro intento.

6. Corregido el reanálisis de segmentos para descripciones situadas fuera del primer chunk de Gemini. El segmento aislado se envía ahora al worker existente con una línea temporal local que empieza en 0 y después se vuelve a asociar a su posición absoluta en el proyecto, evitando `Invalid prepared chunk timeline at chunk 1` sin cambiar la canalización normal de audiodescripción.

7. Se rehízo el reanálisis de segmentos de proyectos guardados para usar los mismos controles de seguridad que la creación completa de audiodescripciones. El fragmento seleccionado usa ahora la misma extracción de audio mono a 16 kHz y el mismo análisis de diálogos/silencios con Pyannote, las mismas reglas temporales y los mismos fallbacks de Gemini, la misma síntesis TTS real con la voz del proyecto y el mismo planificador final para una colocación segura y las pausas extendidas. Al aplicar el segmento, Sonarpad conserva los nuevos tiempos absolutos ya verificados y actualiza la protección de Pyannote para ese fragmento; se eliminó la solución especial anterior de la vista previa para textos reanalizados demasiado largos.

8. Corregido el reemplazo estructural del segmento después del reanálisis completo. Sonarpad ya no exige que el segmento recién verificado tenga exactamente el mismo número de descripciones que el proyecto guardado: las descripciones antiguas de ese chunk se sustituyen por todo el resultado seguro de la canalización completa, por lo que un segmento con 10 descripciones puede pasar correctamente a 9 (o 11). El editor muestra de inmediato la nueva cantidad y los nuevos tiempos para la vista previa, mientras el proyecto y el archivo multimedia guardados no cambian hasta que “Aplicar segmento y volver a exportar MP3” termina correctamente.

9. Se ha rediseñado el reanálisis de segmentos para tratar el fragmento físico seleccionado como una auténtica minipelícula autónoma. Sonarpad pasa ahora esa minipelícula directamente a la misma función `create_audio_description` utilizada por la creación completa normal, de modo que vídeo y pista de audio comparten la misma línea de tiempo local y se ejecutan sin una implementación separada de reanálisis los controles normales de Pyannote, Gemini, TTS real, planificación y seguridad. Solo al terminar esa canalización normal se desplazan los tiempos locales resultantes a la posición original de la película.

Edición de texto
1. Se mejoró “Unir líneas partidas” en Editar > Texto. Sin selección procesa todo el documento; con una selección actúa solo sobre las líneas seleccionadas. Ahora reconstruye párrafos completos uniendo líneas consecutivas incluso cuando una frase termina con puntuación; las líneas en blanco siguen separando párrafos y se conservan las listas numeradas o con viñetas.

10. Corregida la sincronización temporal de la reanalización al volver a situar el minivídeo analizado en la película original. Sonarpad aplica ahora la misma reconciliación pequeña de duración de los segmentos usada por el análisis completo a las descripciones, Visual Evidence y los intervalos de diálogo protegidos, evitando desplazamientos subsegundo que puedan llevar la narración sobre el diálogo.

Versión 0.9.6 – 2026-09-09

1. Corregidos los bloqueos de algunas voces SAPI4 con el seguimiento del cursor activado. El puente espera ahora a que la síntesis termine realmente, manteniendo activo el seguimiento.

2. Corregidos la sincronización entre micrófono y audio del sistema y los clics causados por el redondeo de las marcas de tiempo al grabar podcasts. También se aplica al guardar las fuentes por separado.

3. La síntesis SAPI5 de audiodescripciones se ejecuta ahora en procesos separados. Si falla, se reintenta con menos concurrencia, conservando las descripciones correctas y recordando el límite que funciona para esa voz.
